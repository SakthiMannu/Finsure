...
export class WithdrawComponent {
  ...
  onWithdraw(): void {
    ...
    if (this.amount > 1000000) {
      this.errorMsg = 'Maximum withdrawal limit is &#8377;10,00,000 per transaction'; return;
    }
    ...
    this.loading = true;
    this.errorMsg = '';
    this.api.withdraw({ accountId: this.accountId, amount: this.amount }).subscribe({
      next:  data => { this.loading = false; this.result = data; },
      error: err  => { this.loading = false; this.errorMsg = this.parseError(err, 'Withdrawal failed. Please try again.'); }
    });
  }
  ...
  parseError(err: any, fallback: string): string {
    try {
      ...
      if (err.status === 400)                    return 'Invalid request. Please check the details.';
