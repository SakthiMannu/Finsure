...
public class AccountServiceImpl implements AccountService {
    ...
    public List<AccountResponseDTO> getAccountsByUsername(String email) {


        Member member = memberRepo.findByContactInfo(email)
                .orElseThrow(() -> new RuntimeException(
                    "No member linked to this account. "
                    + "Contact your branch."));

        return accountRepo.findByMember_MemberId(member.getMemberId())
                .stream()
                .map(a -> {
                    AccountResponseDTO dto = acc_map.toResponseDTO(a);
                    dto.setMemberId(a.getMember().getMemberId());
                    return dto;
                })
                .collect(Collectors.toList());
    }
